    ...
    <View className="flex-1">
      ...
      <ScrollView className="mt-16">
        ...
        <View className="flex-row justify-between items-center mx-4 mg-4">
          <View className="border-2 border-yellow-500 rounded-full overflow-hidden">
            <Image
          ...
          <View className="flex-row space-x-4">
            ...
            <TouchableOpacity onPress={() => navigation.navigate("Search")}>
              <MagnifyingGlassIcon size={30} strokeWidth={2} color="#F59E0B" />
            </TouchableOpacity>
          </View>
        ...
        {isTrendingLoading ? (
          ...
          <ScrollView>
            ...
            {/* Popular Movies */}
            {popular?.length > 0 && (
              <PopularMovie title={<Text className="text-yellow-500 text-2xl font-bold mx-4 mb-4">Popular</Text>}
              data={popular} />
            )}

            {/* Top Rated Movies */}
            {topRated?.length > 0 && (
              <TopRatedMovies genre={genre} title={<Text className="text-yellow-500 text-2xl font-bold mx-4 mb-4">Top Rated</Text>} data={topRated} />
            )}

            {/* Upcoming Movies */}
            {upcoming?.length > 0 && (
              <UpcomingMovie title={<Text className="text-yellow-500 text-2xl font-bold mx-4 mb-4">Upcoming</Text>} data={upcoming} />
            )}
          </ScrollView>
        )}
      </ScrollView>
